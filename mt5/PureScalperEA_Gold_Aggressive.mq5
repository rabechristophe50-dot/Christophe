//+------------------------------------------------------------------+
//|                               PureScalperEA_Gold_Aggressive.mq5   |
//|        Expert Advisor de Scalping AGRESSIF pour l'OR - 100% Auto  |
//|                                                                  |
//|  Version XAUUSD agressive, calibree IC Markets Raw (M1).         |
//|  Plus de trades : EMA rapides, RSI large, ATR bas, 2 positions.  |
//|  1 pip = 0.01 $ (1 cent).                                        |
...
//+------------------------------------------------------------------+
#property copyright "PureScalperEA Gold Aggressive"
#property version   "1.00"
#property strict
#property description "Scalping AGRESSIF pour l'OR (XAUUSD M1) - calibre IC Markets Raw. Plus de trades."
...
input string   InpComment          = "PureScalperGoldAggr"; // Commentaire des ordres

input group    "=== Gestion du risque ==="
input bool     InpUseMoneyMgmt     = true;        // Lot dynamique base sur le risque
input double   InpRiskPercent      = 0.5;         // Risque par trade (% du capital) - or = volatil
input double   InpFixedLot         = 0.01;        // Lot fixe (si money mgmt desactive)
input double   InpMaxLot           = 5.0;         // Lot maximum autorise
input int      InpMaxPositions     = 2;           // Nombre max de positions simultanees (agressif)

input group    "=== Indicateurs ==="
input int      InpTrendEMA         = 200;         // EMA filtre de tendance
input int      InpFastEMA          = 5;           // EMA rapide (agressif)
input int      InpSlowEMA          = 13;          // EMA lente (agressif)
input int      InpRSIPeriod        = 14;          // Periode RSI
input double   InpRSIBuyMax        = 80.0;        // RSI max pour acheter (large = agressif)
input double   InpRSIBuyMin        = 45.0;        // RSI min pour acheter (milieu bande achat)
input double   InpRSISellMin       = 20.0;        // RSI min pour vendre (large = agressif)
input double   InpRSISellMax       = 55.0;        // RSI max pour vendre (milieu bande vente)
input int      InpATRPeriod        = 14;          // Periode ATR
input double   InpATRMinPips       = 40.0;        // ATR minimum en pips (agressif : 40 pips = 0.40 $)
...
input double   InpBreakEvenPips     = 120.0;      // Profit avant break-even (120 pips = 1.20 $)
input double   InpBreakEvenLock     = 20.0;       // Pips verrouilles au break-even (0.20 $)
input bool     InpUseTrailing      = true;        // Activer le trailing stop
input double   InpTrailStartPips    = 180.0;      // Profit avant trailing (180 pips = 1.80 $)
input double   InpTrailStepPips     = 100.0;      // Distance du trailing (100 pips = 1.00 $)

input group    "=== Filtres de trading ==="
input double   InpMaxSpreadPips    = 25.0;        // Spread max (IC Markets Raw : 25 pips = 0.25 $)
input bool     InpUseTimeFilter    = true;        // Activer le filtre horaire
input int      InpStartHour        = 7;           // Heure de debut (serveur)
input int      InpEndHour          = 21;          // Heure de fin (serveur)
...
void OnTick()
  {
   ...
   bool rsiBuyOK  = (rsi < InpRSIBuyMax) && (rsi > InpRSIBuyMin);
   ...
   bool rsiSellOK = (rsi > InpRSISellMin) && (rsi < InpRSISellMax);
